...
struct Masina* copiazaPrimeleNElemente(struct Masina* vector, int nrElemente, int nrElementeCopiate) {
	...
	if (vector != NULL && nrElemente >= nrElementeCopiate && nrElementeCopiate > 0)
	{
	...
	if (vector != NULL && nrElemente > 0)
	{
		...
		for (int i = 0; i < nrElemente; i++)
		{
			if (vector[i].kilometriiParcursi > prag)
			{
				...
				(*vectorNou)[k].sofer = malloc(strlen(vector[i].sofer) + 1);
...
struct Masina getPrimaMasinaDupaSofer(struct Masina* vector, int nrElemente, const char* sofer) {
